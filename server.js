...
app.post("/students/:id/result", async (req, res) => {
    try {
        ...
        const data = await pool.query(
            "SELECT mark1, mark2, mark3 FROM students WHERE id = $1",
            [id]
        );
        const name = await pool.query("SELECT name FROM students WHERE id = $1", [
            id,
        ]);
...
app.get("/students", async (req, res) => {
    ...
    if (query === "passed") {
        const name = await pool.query(
            "SELECT name FROM students where mark1 > 22 AND mark2 > 22 AND mark3 > 22"
        );
        ...
        const name = await pool.query(
            "SELECT name FROM students where mark1 <= 22 OR mark2 <= 22 OR mark3 <= 22"
        );
        const studentName = name.rows;
        res.status(200).json({
            studentName,
